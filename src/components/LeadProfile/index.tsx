...
import { LayoutDashboard, FileText, PenTool, Wallet, History, CreditCard, Activity, Loader2, Save, Trash2, Server } from 'lucide-react';
...
export default function LeadProfilePage({ lead, onBack }: LeadProfilePageProps) {
  const [activeTab, setActiveTab] = useState<'overview' | 'resume' | 'update' | 'platform'>(() => {
    const saved = localStorage.getItem('crm_profile_active_tab');
    return (saved as 'overview' | 'resume' | 'update' | 'platform') || 'overview';
  });
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-300 p-6 max-w-400 mx-auto">
      ...
      <div className="flex items-center gap-2 mb-6 border-b border-white/10 pb-1">
        ...
        <TabButton active={activeTab === 'resume'} onClick={() => setActiveTab('resume')} icon={<FileText size={16}/>} label="KYC Profile" />
        <TabButton active={activeTab === 'update'} onClick={() => setActiveTab('update')} icon={<PenTool size={16}/>} label="Update / Upload" />
        <TabButton active={activeTab === 'platform'} onClick={() => setActiveTab('platform')} icon={<Server size={16}/>} label="Platform Access" />
      </div>
        ...
        <div className="grid grid-cols-12 gap-6 animate-in fade-in duration-300">
          ...
          <div className="col-span-12 lg:col-span-8 space-y-6">
            <div className="grid grid-cols-3 gap-4">
              <div className="glass-panel p-6 rounded-xl border border-white/5 relative overflow-hidden group">
                ...
                <div className="mt-4 flex gap-2">
                  <button className="cursor-pointer flex-1 bg-green-600 hover:bg-green-500 text-white text-xs font-bold py-2 rounded-lg transition">DEPOSIT</button>
                  <button className="cursor-pointer flex-1 bg-white/5 hover:bg-white/10 text-white text-xs font-bold py-2 rounded-lg border border-white/10 transition">WITHDRAW</button>
                </div>
              </div>
            ...
            <div className="glass-panel p-6 rounded-xl border border-white/5 min-h-100">
               <h3 className="text-lg font-bold text-white mb-4 flex items-center gap-2"><History size={18} className="text-blue-500" /> Trade History</h3>
               <div className="w-full h-64 border-2 border-dashed border-white/10 rounded-xl flex items-center justify-center text-gray-500 text-sm italic">Waiting for Platform connection...</div>
            </div>
          </div>
          ...
          <div className="col-span-12 lg:col-span-4 space-y-6">
            <div className="glass-panel p-6 rounded-xl border border-white/5 h-full flex flex-col">
              <h3 className="text-lg font-bold text-white mb-4 flex items-center gap-2"><FileText size={18} className="text-yellow-500" /> Agent Notes</h3>
              <div className="relative">
                ...
                <button 
                    ...
                    className="cursor-pointer w-full mt-3 bg-blue-600 hover:bg-blue-500 disabled:bg-blue-600/50 text-white font-bold py-3 rounded-xl transition text-xs uppercase tracking-widest flex items-center justify-center gap-2"
                >
              ...
              <div className="mt-8 space-y-4 flex-1 overflow-y-auto max-h-125 pr-2 custom-scrollbar">
                {loadingNotes ? ( <div className="text-center text-gray-500 py-4"><Loader2 className="animate-spin mx-auto"/></div> ) 
                : notes.length === 0 ? ( <div className="text-center text-gray-600 text-xs italic py-4">No notes yet.</div> ) 
                : (
                    notes.map((note) => (
                        <div key={note.id} className="p-4 bg-white/5 rounded-xl border-l-2 border-blue-500 animate-in fade-in slide-in-from-right-4 duration-300 group relative">
                            ...
                            {canDelete && (
                                <button 
                                    ...
                                    className="cursor-pointer absolute top-2 right-2 p-1.5 rounded-lg bg-red-500/10 text-red-500 opacity-0 group-hover:opacity-100 transition hover:bg-red-500/20"
                                >
                                    <Trash2 size={14} />
                                </button>
                            )}
...
// FIXED: Added 'cursor-pointer' explicitly here
function TabButton({ active, onClick, icon, label }: any) {
    return (
        <button 
          onClick={onClick}
          className={`cursor-pointer flex items-center gap-2 px-4 py-2 text-sm font-bold rounded-t-lg transition-all duration-200 border-b-2 ${active ? 'text-cyan-400 border-cyan-400 bg-cyan-500/5' : 'text-gray-400 border-transparent hover:text-white hover:bg-white/5'}`}
        >
          {icon}
          {label}
        </button>
