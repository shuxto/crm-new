import { User, MapPin, Briefcase, ShieldCheck, ShieldAlert, Shield, Fingerprint, Check, FileText, Landmark, CreditCard, FileBox, Loader2, AlertCircle, FolderOpen, ExternalLink, Info } from 'lucide-react';
...
export default function KYCSummary({ lead }: Props) {
  ...
  const status = lead?.kyc_status || 'Not Verified'; 

  const getStatusBadge = () => {
    switch (status) {
      case 'Approved': return { icon: <ShieldCheck size={20} />, color: 'text-emerald-400', bg: 'bg-emerald-500/10', border: 'border-emerald-500/20' };
      case 'Pending': return { icon: <Shield size={20} />, color: 'text-amber-400', bg: 'bg-amber-500/10', border: 'border-amber-500/20' };
      default: return { icon: <ShieldAlert size={20} />, color: 'text-rose-400', bg: 'bg-rose-500/10', border: 'border-rose-500/20' };
    }
  };
                ...
                <div className="flex items-center gap-5 relative z-10">
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-2 text-sm text-gray-400">
                            <span>Status:</span>
                            <span className={`font-bold uppercase ${badge.color}`}>{status}</span>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
      ...
      <div className="glass-panel p-6 rounded-2xl border border-white/5 flex items-center justify-between relative overflow-hidden">
        ...
        <button 
            ...
            className="flex items-center gap-3 px-4 py-2 rounded-lg bg-black/20 border border-white/10 hover:bg-white/5 transition"
        >
            {copied ? <Check size={16} className="text-green-400"/> : <Fingerprint size={16} className="text-gray-400"/>}
            <span className="text-xs font-mono text-gray-300">{copied ? 'Copied!' : lead?.id}</span>
        </button>
      </div>
      ...
      <div className="grid grid-cols-12 gap-6">
          ...
          {/* LEFT COLUMN: Identity Card Only */}
          ...
          {/* RIGHT COLUMN: Address, Financial, Documents, THEN Notes */}
          <div className="col-span-12 lg:col-span-8 grid grid-cols-2 gap-6 h-fit">
              ...
              {/* Additional Information Display (MOVED TO BOTTOM) */}
